...
{
    RichTextViewController * ctrl=[[RichTextViewController alloc]init];
...
-(void)initView
{
    self.textView = [[UITextView alloc]initWithFrame:CGRectMake(0, 0.0f, 300.0f, 400.0f)];
    self.textView.backgroundColor = [UIColor redColor];
    [self.view addSubview:self.textView];
    self.textView.delegate= self;
    
    self.imageBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    self.imageBtn.frame = CGRectMake(375-100, 667 - 280, 50, 30);
    [self.imageBtn setTitle:@"选择图片" forState:UIControlStateNormal];
    ...
    self.placeholderLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 64, 300.0f, 30.0f)];
    self.placeholderLabel.text = @"请输入内容";
    self.placeholderLabel.backgroundColor = [UIColor cyanColor];
    [self.view addSubview:self.placeholderLabel];
    
    self.finishBtn =[UIButton buttonWithType:UIButtonTypeSystem];
    self.finishBtn.frame = CGRectMake(375 - 50, 667 - 280, 50, 30);
...
- (void)onKeyboardNotification:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        [UIView animateWithDuration:0.29 animations:^{
            self.finishBtn.alpha = 0.0f;
            self.imageBtn.alpha = 0.0f;
        } completion:^(BOOL finished) {
