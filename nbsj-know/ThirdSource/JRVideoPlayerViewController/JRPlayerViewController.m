...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (touchX < (1.0/4 * screenWidth) && offsetY != 0) {
        // 上下滑动改变亮度
    ...
    } else if (touchX > (3.0/4 * screenWidth) && offsetY != 0) {
        // 上下滑动改变音量
        if (sysVolume + delta > 0.0 && sysVolume + delta < 1.0) {
            [volumeSlider setValue:sysVolume + delta]; // 设置音量
        }
        
    } else if (touchX > (1.0/4 * screenWidth) && touchX < (3.0/4 * screenWidth) && offsetX != 0) {
        // 中屏幕中间左右滑动改变进度
